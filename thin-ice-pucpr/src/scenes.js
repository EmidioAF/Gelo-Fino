...
class SceneInstructions {
  ...
  draw(p) {
    ...
    const linhas = [
      ...
      ['  Não caia na água e não deixe o tempo', false],
      ['  acabar!', false],
      ['', false],
      ['❄ TIPOS DE BLOCO', true],
      ['  ❄ Gelo Normal  — 1 passo  (+10 pts)', false],
      ['  ❄❄ Reforçado  — 2 passos (+10 pts)', false],
      ['  ⚡ Frágil       — 1 passo  (+10 pts)', false],
      ['  ★ Bônus         — 1 passo  (+60 pts)', false],
...
class SceneGameOver {
  ...
  enter({ reason } = {}) {
    this.reason = reason === 'timeout' ? '⏱ Tempo Esgotado!' : '💧 Você Caiu!';
    this.animT  = 0;
